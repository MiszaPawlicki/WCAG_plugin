const criteria = [
  {
    name: "Non-text Content",
    ref: "1.1.1",
    level: "A",
    impacts: ["Auditory", "Visual"],
  },
  {
    name: "Audio-only and Video-only (Prerecorded)",
    ref: "1.2.1",
    level: "A",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Captions (Prerecorded)",
    ref: "1.2.2",
    level: "A",
    impacts: ["Auditory", "Cognitive"],
  },
  {
    name: "Audio Description or Media Alternative (Prerecorded)",
    ref: "1.2.3",
    level: "A",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Captions (Live)",
    ref: "1.2.4",
    level: "AA",
    impacts: ["Auditory", "Cognitive"],
  },
  {
    name: "Audio Description (Prerecorded)",
    ref: "1.2.5",
    level: "AA",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Sign Language (Prerecorded)",
    ref: "1.2.6",
    level: "AAA",
    impacts: ["Auditory"],
  },
  {
    name: "Extended Audio Description (Prerecorded)",
    ref: "1.2.7",
    level: "AAA",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Media Alternative (Prerecorded)",
    ref: "1.2.8",
    level: "AAA",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Audio-only (Live)",
    ref: "1.2.9",
    level: "AAA",
    impacts: ["Auditory"],
  },
  {
    name: "Info and Relationships",
    ref: "1.3.1",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Meaningful Sequence",
    ref: "1.3.2",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Sensory Characteristics",
    ref: "1.3.3",
    level: "A",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Orientation",
    ref: "1.3.4",
    level: "AA",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Identify Input Purpose",
    ref: "1.3.5",
    level: "AA",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Identify Purpose",
    ref: "1.3.6",
    level: "AAA",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Use of Color",
    ref: "1.4.1",
    level: "A",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Audio Control",
    ref: "1.4.2",
    level: "A",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Contrast (Minimum)",
    ref: "1.4.3",
    level: "AA",
    impacts: ["Visual"],
  },
  {
    name: "Resize text",
    ref: "1.4.4",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Images of Text",
    ref: "1.4.5",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Contrast (Enhanced)",
    ref: "1.4.6",
    level: "AAA",
    impacts: ["Visual"],
  },
  {
    name: "Low or No Background Audio",
    ref: "1.4.7",
    level: "AAA",
    impacts: ["Auditory", "Cognitive"],
  },
  {
    name: "Visual Presentation",
    ref: "1.4.8",
    level: "AAA",
    impacts: ["Cognitive"],
  },
  {
    name: "Images of Text (No Exception)",
    ref: "1.4.9",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Reflow",
    ref: "1.4.10",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Non-text Contrast",
    ref: "1.4.11",
    level: "AA",
    impacts: ["Visual"],
  },
  {
    name: "Text Spacing",
    ref: "1.4.12",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Content on Hover or Focus",
    ref: "1.4.13",
    level: "AA",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Keyboard",
    ref: "2.1.1",
    level: "A",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "No Keyboard Trap",
    ref: "2.1.2",
    level: "A",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Keyboard (No Exception)",
    ref: "2.1.3",
    level: "AAA",
    impacts: ["Motor"],
  },
  {
    name: "Character Key Shortcuts",
    ref: "2.1.4",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Timing Adjustable",
    ref: "2.2.1",
    level: "A",
    impacts: ["Auditory", "Cognitive", "Motor", "Visual"],
  },
  {
    name: "Pause, Stop, Hide",
    ref: "2.2.2",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "No Timing",
    ref: "2.2.3",
    level: "AAA",
    impacts: ["Auditory", "Cognitive", "Motor", "Visual"],
  },
  {
    name: "Interruptions",
    ref: "2.2.4",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Re-authenticating",
    ref: "2.2.5",
    level: "AAA",
    impacts: ["Auditory", "Cognitive", "Motor", "Visual"],
  },
  {
    name: "Timeouts",
    ref: "2.2.6",
    level: "AAA",
    impacts: ["Auditory", "Cognitive", "Motor", "Visual"],
  },
  {
    name: "Three Flashes or Below Threshold",
    ref: "2.3.1",
    level: "A",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Three Flashes",
    ref: "2.3.2",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Animation from Interactions",
    ref: "2.3.3",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Bypass Blocks",
    ref: "2.4.1",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Page Titled",
    ref: "2.4.2",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Focus Order",
    ref: "2.4.3",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Link Purpose (In Context)",
    ref: "2.4.4",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Multiple Ways",
    ref: "2.4.5",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Headings and Labels",
    ref: "2.4.6",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Focus Visible",
    ref: "2.4.7",
    level: "AA",
    impacts: ["Cognitive", "Motor"],
  },
  {
    name: "Location",
    ref: "2.4.8",
    level: "AAA",
    impacts: ["Cognitive"],
  },
  {
    name: "Link Purpose (Link Only)",
    ref: "2.4.9",
    level: "AAA",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Section Headings",
    ref: "2.4.10",
    level: "AAA",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Focus Not Obscured (Minimum)",
    ref: "2.4.11",
    level: "AA",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Focus Not Obscured (Enhanced)",
    ref: "2.4.12",
    level: "AAA",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Focus Appearance",
    ref: "2.4.13",
    level: "AAA",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Pointer Gestures",
    ref: "2.5.1",
    level: "A",
    impacts: ["Cognitive", "Motor"],
  },
  {
    name: "Pointer Cancellation",
    ref: "2.5.2",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Label in Name",
    ref: "2.5.3",
    level: "A",
    impacts: ["Motor", "Visual"],
  },
  {
    name: "Motion Actuation",
    ref: "2.5.4",
    level: "A",
    impacts: ["Motor"],
  },
  {
    name: "Target Size",
    ref: "2.5.5",
    level: "AAA",
    impacts: ["Motor"],
  },
  {
    name: "Concurrent Input Mechanisms",
    ref: "2.5.6",
    level: "AAA",
    impacts: ["Motor"],
  },
  {
    name: "Dragging Movements",
    ref: "2.5.7",
    level: "AA",
    impacts: ["Motor"],
  },
  {
    name: "Target Size (Minimum)",
    ref: "2.5.8",
    level: "AA",
    impacts: ["Motor"],
  },
  {
    name: "Language of Page",
    ref: "3.1.1",
    level: "A",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Language of Parts",
    ref: "3.1.2",
    level: "AA",
    impacts: ["Auditory", "Cognitive", "Visual"],
  },
  {
    name: "Unusual Words",
    ref: "3.1.3",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Abbreviations",
    ref: "3.1.4",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Reading Level",
    ref: "3.1.5",
    level: "AAA",
    impacts: ["Cognitive"],
  },
  {
    name: "Pronunciation",
    ref: "3.1.6",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "On Focus",
    ref: "3.2.1",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "On Input",
    ref: "3.2.2",
    level: "A",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Consistent Navigation",
    ref: "3.2.3",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Consistent Identification",
    ref: "3.2.4",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Change on Request",
    ref: "3.2.5",
    level: "AAA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Consistent Help",
    ref: "3.2.6",
    level: "A",
    impacts: ["Cognitive"],
  },
  {
    name: "Error Identification",
    ref: "3.3.1",
    level: "A",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Labels or Instructions",
    ref: "3.3.2",
    level: "A",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Error Suggestion",
    ref: "3.3.3",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
  {
    name: "Error Prevention (Legal, Financial, Data)",
    ref: "3.3.4",
    level: "AA",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Help",
    ref: "3.3.5",
    level: "AAA",
    impacts: ["Cognitive", "Motor"],
  },
  {
    name: "Error Prevention (All)",
    ref: "3.3.6",
    level: "AAA",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Redundant Entry",
    ref: "3.3.7",
    level: "A",
    impacts: ["Cognitive", "Motor"],
  },
  {
    name: "Accessible Authentication (Minimum)",
    ref: "3.3.8",
    level: "AA",
    impacts: ["Cognitive"],
  },
  {
    name: "Accessible Authentication (Enhanced)",
    ref: "3.3.9",
    level: "AAA",
    impacts: ["Cognitive"],
  },
  {
    name: "Parsing",
    ref: "4.1.1",
    level: "Removed",
    impacts: [],
  },
  {
    name: "Name, Role, Value",
    ref: "4.1.2",
    level: "A",
    impacts: ["Cognitive", "Motor", "Visual"],
  },
  {
    name: "Status Messages",
    ref: "4.1.3",
    level: "AA",
    impacts: ["Cognitive", "Visual"],
  },
];

// a function to prepare the guidline name to be concatinated with the wcag url
// makes all chars lower, replaces spaces with dashes, and removes parenthesis
function linkConversion(inputString) {
  return inputString.toLowerCase().replace(/ /g, "-").replace(/[()]/g, "");
}

const wcagpath = "https://www.w3.org/WAI/WCAG22/Understanding";

//function to identify guideline and return wcag reference
function wcagify(string) {
  console.log("wcagify in background.js called with string:", string);
  const notFound = new Error(`${string}: No WCAG reference found`);
  const wcagMatch = string.match(/(\d*\.\d*\.\d*)/);

  if (!wcagMatch) throw notFound;

  const ref = wcagMatch[1];
  let name, slug, level, impacts;

  criteria.forEach((criterion) => {
    if (criterion.ref === ref) {
      name = criterion.name;
      slug = linkConversion(criterion.name);
      level = criterion.level;
      impacts = criterion.impacts;
    }
  });

  if (name === undefined) throw notFound;

  return {
    criterion: `${ref} - ${name}`,
    ref,
    name,
    link: `${wcagpath}/${slug}.html`,
    level,
    impacts,
  };
}

// send output of wcagify to popup.js
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "callWCAGify") {
    // Call the function with data from the popup
    const result = wcagify(request.data);
    sendResponse({ action: "backgroundResponse", data: result });
  }
});
